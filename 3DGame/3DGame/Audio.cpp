...
IXAudio2SubmixVoice*			CAudio::m_pSFXSubmixVoice;		//サブミックスインターフェース
...
IXAudio2SubmixVoice*			CAudio::m_pSESFXSubmixVoice[32];		//サブミックスインターフェース
...
unsigned char* CAudio::LoadOgg(ChunkInfo* p_chunk_info, WAVEFORMATEX* p_wave, const char* name)
{
	...
	while (1)
	{
		...
		memcpy(&buffer[comSize], &tmpBuffer[0], readSize);//バッファーにコピー
		comSize += readSize;
	}
...
void CAudio::DeleteAudio()
{
	...
	for (int i = 0; i < 32; i++)
	{
		for (int j = 0; j < 16; j++)
		{
			if (m_pSESourceVoice[i] != nullptr)
			{
				m_pSESourceVoice[i][16]->Stop();
				m_pSESourceVoice[i][16]->FlushSourceBuffers();
				m_pSESourceVoice[i][16]->DestroyVoice();
			}
		}
...
//指定したチャンクを配列から探し出して、チャンク以下に設定されているデータサイズとデー
//タ部の先頭アドレスを返す。また、必ずチャンクは見つかるとして簡略化している

...
typedef class CAudio
{
	public:
		...
		static void LoadSEMusic(int id, const char* name);//SE用の音楽読み込み
	...
	private:
		static unsigned char* LoadWave(ChunkInfo* p_chunk_info, WAVEFORMATEX* p_wave, const wchar_t* name);//Wave
		static unsigned char* LoadOgg(ChunkInfo* p_chunk_info, WAVEFORMATEX* p_wave, const char* name);//Ogg
		...
		static IXAudio2SubmixVoice* m_pSFXSubmixVoice;//サブミックスインターフェース
		...
		static IXAudio2SubmixVoice* m_pSESFXSubmixVoice[32];//SE用サブミックスインターフェース

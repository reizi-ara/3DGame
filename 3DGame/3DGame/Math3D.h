...
namespace Math3D
{
	...
	void Transpose(float mat[16], float dest[16]);//転置行列関数
	...
	void Translate(float mat[16], float vec[3], float dest[16]);//平行移動行列関数
	void Rotate(float mat[16], float angle, float axis[3], float dest[16]);//回転行列

...
ID3D11ShaderResourceView* CDraw2DPolygon::LoadImage(const wchar_t* img_name)
{
	...
	//設定を元にテクスチャー作成
	DirectX::CreateWICTextureFromFile(Dev::GetDevice(), Dev::GetDeviceContext(), img_name, nullptr, &view, 0U);

	if (FAILED(hr))
	{
		MessageBox(0, L"テクスチャが読み込みできませんでした", NULL, MB_OK);
		return nullptr;
	}
	return view;
}
...
HRESULT CDraw2DPolygon::InitPolygonRender()
{
	...
	//hlslファイル読み込み　ブロブ作成とはシェーダーの塊みたいなもの
	...
	//コンパイルしたバーテックスシェーダーヲもとにインターフェイスを作成
	...
	if (FAILED(hr))
	{
		MessageBox(0, L"レイアウト作成を失敗", NULL, MB_OK);
		return hr;
	}
	SAFE_RELEASE(pCompiledshader);
	...
	//ステータスとバッファに入れる元データをもとにインデックスバッファ作成
	...
	//バッファにコンスタントバッファ(シェーダにデータの受け渡し用)ステータスを設定
